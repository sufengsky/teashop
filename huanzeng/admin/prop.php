...
if ( $step == "copy" )
{
				$msql->query( "delete from {P}_hz_prop where catid='{$catid}'" );
				$p = 1;
				$msql->query( "select * from {P}_hz_prop where catid='{$pid}'" );
				while ( $msql->next_record( ) )
				{
								$propname = $msql->f( "propname" );
								$xuhao = $msql->f( "xuhao" );
								$fsql->query( "insert into {P}_hz_prop values(\r\n\t\t\t0,\r\n\t\t\t'{$catid}',\r\n\t\t\t'{$propname}',\r\n\t\t\t'{$xuhao}'\r\n\t\t\t)" );
								$p++;
				}
}
if ( $step == "add" )
{
				$msql->query( "select count(id) from {P}_hz_prop where catid='{$catid}'" );
				if ( $msql->next_record( ) )
				{
				...
				$msql->query( "select max(xuhao) from {P}_hz_prop where catid='{$catid}' " );
				if ( $msql->next_record( ) )
				{
								$max = $msql->f( "max(xuhao)" );
				}
				$max = $max + 1;
				$msql->query( "insert into {P}_hz_prop values(\r\n0,\r\n'{$catid}',\r\n'{$propname}',\r\n'{$max}'\r\n)" );
}
if ( $step == "modify" )
{
				$msql->query( "update {P}_hz_prop set propname='{$propname}',xuhao='{$xuhao}' where id='{$id}'" );
}
if ( $step == "del" )
{
				$msql->query( "delete from {P}_hz_prop where id='{$id}'" );
}
